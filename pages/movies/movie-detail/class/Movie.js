...
class Movie {
    ...
    getMovieData(cb) {
        ...
        this.cb = cb;
        // bind(this)的作用是把getMovieData函数体内代表Movie实例对象的this绑定到processDoubanData内部环境的上下文，使得processDoubanData中this.cb(movie)的this成为了Movie实例对象的this环境，变成这里的this.cb
    ...
    processDoubanData(data) {
        ...
        var movie = {
            ...
            // cb不是这里内部的参数，要想拿到外部的this.cb，通过util.http(this.url, this.processDoubanData.bind(this))，把getMovieData函数体内代表Movie实例对象的this绑定到processDoubanData内部环境的上下文，使得processDoubanData中this.cb(movie)的this成为了Movie实例对象的this环境，this.cb即为getMovieData中的this.cb
